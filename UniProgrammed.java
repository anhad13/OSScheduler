...
public class UniProgrammed{
	public static void run(Queue<Process> inqueue, Scanner randomfile, boolean is_verbose)
	{
		...
		while(done < todo)
		{
			...
			{
				System.out.printf("\n");
				System.out.printf("Before Cycle: "+Integer.toString(clock)+"\t");
			...
			if(current_process==null)
			{
				if(io_table.size()==0 && todoP==null)
				{
					current_process=readyqueue.poll();
					todoP=current_process;
				}
				else if(io_table.size()==0)
					current_process=todoP;
				else
					current_process=null;
				...
				int number=randomfile.nextInt();
				int burst=current_process.get_burst(number);
